...
def coinToss(trueProb = 0.5):
	rand = random.WichmannHill().random()
		...
		while i < num:
			...
			if coinToss() and len(self.elements) > 1:
				...
				if len(self.matDict[randElem]) < 1:
					...
					matName = random.WichmannHill().sample(self.matDict[randElem], 1)[0]
...
# returns a small box (random.WichmannHill()ly) of given size from inside the container box
...
# Generating the data
labels, targets = GenerateData(5000)
allWords = set()
